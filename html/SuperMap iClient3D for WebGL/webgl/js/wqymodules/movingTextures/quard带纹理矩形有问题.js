...
function quard(options) {
    ...
    this._canvas.addEventListener('webglcontextlost', function(e) {
        contextlost=true;
        // that._gl.getExtension('WEBGL_lose_context').restoreContext();
    }, false);
...
quard.prototype.draw = function () {
    if(initImageSuccess!==true) return;
    if(contextlost===true) return ;

    let result=initShaders(this._gl, vs, fs);
    if(result<0)
        return false;
    var n = initVertexBuffer(this._gl);
    if (n < 0) {
        console.log('顶点写入缓存失败！');
        return false;
    }

    initTextures(this._gl,n);
    // this._gl.drawArrays(this._gl.POINTS,0,1);
    //开始画点
    this._gl.drawArrays(this._gl.TRIANGLE_STRIP, 0, n);
}
...
function initTextures(gl, n) {
    ...
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGB, gl.RGB, gl.UNSIGNED_BYTE, image) //纹理图
