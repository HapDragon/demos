...
//void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
		...
		vec2 resolution = u_resolution.xy;
		 ...
		 vec2 fragCoord = gl_FragCoord.xy/u_resolution.xy*vec2(500.,400.);
		 fragCoord.x=500.0-fragCoord.x;
	...
	float xfuel = 1.0-abs(2.0*xpart-1.0);//pow(1.0-abs(2.0*xpart-1.0),0.5);
	 float xfuel2= pow(1.0-abs(2.0*xpart-1.0),0.3);
	//
	float timeSpeed = 0.5;
	...
	float ypart2=1.4-ypart;
    if(ypart2>1.0) ypart2=1.0;
	//vec3 smoke = vec3(0.3*pow(xfuel,3.0)*pow(ypart,2.0)*(smokeNoise+0.4*(1.0-noise)));
	vec3 smoke = vec3(0.3*pow(xfuel2,3.0)*pow(ypart2,3.0)*(smokeNoise+0.8*(1.0-noise)))*1.5;
	//vec3 smoke = vec3(0.3*pow(1.0-ypart,2.0))*2.0;
	//
	// sparks
	...
	//
	fire=max(fire,sparks);
	//vec4 fragColor = vec4(fire,length(fire)*1.2);
	vec4 fragColor = vec4(fire+smoke,length(fire+smoke));

	//if(fire.x+fire.y+fire.z<0.4||smokeNoise<0.05){
       // 	discard;
        //	}

	//vec4 fragcol = vec4( color, pow(max(color,0.),2.)*0.4, pow(max(color,0.),3.)*0.15 , 1.0);
